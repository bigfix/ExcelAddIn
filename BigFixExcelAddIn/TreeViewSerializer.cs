...
namespace TreeViewSerialization
{
	...
	public class TreeViewSerializer
	{
		...
		public void SerializeTreeView(TreeView treeView, string fileName) 
		{
			...
			textWriter.WriteStartElement("FixletType");
		...
		public void DeserializeTreeView(TreeView treeView, string fileName, string besTag)
		{
			...
			try
			{
                ...
                Stream str = asm.GetManifestResourceStream(asm.GetName().Name + "." + fileName);
                // MessageBox.Show("Assembly name: " + asm.GetName().Name);

                if (str == null)
                    throw new Exception("Could not locate embedded resource '" + fileName + "' in assembly '" + asm.GetName().Name + "'");
                // End Lee Wei
